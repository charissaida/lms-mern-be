...
const exportUsersReport = async (req, res) => {
  try {
    ...
    userTasks.forEach((task) => {
      if (task.assignedTo && userTasksMap[task.assignedTo._id]) {
        userTasksMap[task.assignedTo._id].taskCount += 1;
        if (task.status === "Pending") {
          userTasksMap[task.assignedTo._id].pendingTasks += 1;
        } else if (task.status === "In Progress") {
          userTasksMap[task.assignedTo._id].inProgressTasks += 1;
        } else if (task.status === "Completed") {
          userTasksMap[task.assignedTo._id].completedTasks += 1;
        }
      }
    });
